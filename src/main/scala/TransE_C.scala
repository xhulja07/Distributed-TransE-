...
object TransE_C extends App {
  ...
  scalar(CONFIG) {
    ...
    calls.add(() => {
      ...
      var lineNr: Int = 0
      val stream: InputStream = getClass.getResourceAsStream("/FB15k/entity2id.txt")
    ...
    calls.add(() => {
      ...
      var lineNr: Int = 0
      val stream: InputStream = getClass.getResourceAsStream("/FB15k/relation2id.txt")
      for (line <- Source.fromInputStream(stream).getLines()) {
        lineNr += 1
        val res = line.split("\t")
        smtr.addData(res(0), Integer.valueOf(res(1)))
    ...
    val tripleNr = populateTrainingTriplesCache("/FB15k/train.txt", tripleIDs, filterCache, entitycache, relationcache)
    println(" Train triples read")
    val validTriples = populateTrainingTriplesCache("/FB15k/valid.txt", tripleIDs, filterCache, entitycache, relationcache)
    println(" Valid triples read")
    testTriples = populateTestCache("/FB15k/test.txt", testCache, entitycache, relationcache)
